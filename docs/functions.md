...
利用者の目的と行動は[利用者の行動シナリオ](user-action-scenarios.md)、画面上の表現と動線は[UI設計](ui-design.md)、扱う概念は[概念データモデル](conceptual-data-model.md)、実装順は[開発ロードマップ](roadmap.md)を正本とする。
...
- 入力：アプリの起動、または新規作成の要求
- 処理：本文が空で保存先を持たない文書を、編集対象として開始する。
...
- 入力：保存先のない文書の保存要求、または別名保存の要求
- 処理：利用者が保存場所とファイル名を選択またはキャンセルできるようにする。
...
- 出力：保存の成功と保存先。成功後、そのファイルを現在の文書の正本とする。
...
- 処理：本文を選ばれた保存先へ保存する。
- 出力：保存の成功と新しい保存先。成功後、新しいファイルを現在の文書の正本とする。
...
- 入力：アプリの起動
- 処理：ユーザー文書とは分離されたLetteraのアプリデータから、保存済みの基本フォントサイズとカラーモードを取得する。
...
- 処理：基本フォントサイズが利用可能な範囲にあり、カラーモードがsystem、light、darkのいずれかであることを確認する。
- 出力：画面へ適用可能な設定。不正または未知の値には対応する既定値を使用する。
- 未決事項：基本フォントサイズの既定値、最小値、最大値、変更単位。

### FN-27 アプリ設定を変更する

- 入力：検証済みの基本フォントサイズまたはカラーモード
